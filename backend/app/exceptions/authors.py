...
class AuthorNotFound(exceptions.NotFound):
    code = 3001
    message = _("Author ID %(author_id)s not found.")


class AuthorAlreadyExists(exceptions.ExistsError):
    code = 3002
    message = _("Author ID %(author_id)s already exists.")


class AuthorCreationError(exceptions.CreateError):
    code = 3003
    message = _("Author ID %(author_id)s create failed")


class AuthorDeletionError(exceptions.DeleteError):
    code = 3004
    message = _("Author ID %(author_id)s delete failed")
